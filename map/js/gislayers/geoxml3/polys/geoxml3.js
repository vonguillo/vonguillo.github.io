...
geoXML3.parser = function (options) {
    ...
    var render = function (responseXML, doc) {
        ...
        if (!responseXML || responseXML == "failed parse") {
            ...
            for (pm = 0; pm < placemarkNodes.length; pm++) {
                ...
                var extDataNodes = node.getElementsByTagName('ExtendedData');//getElementsByTagName(node, 'ExtendedData');
                if (!!extDataNodes && extDataNodes.length > 0) {
                    ...
                    for (var d = 0; d < dataNodes.length; d++) {
                        ...
                        if (name == 'description') {
                            placemark.vars.val[name] = "<h2>" + placemark.vars.val['name'] + "</h2></br>" + val
                        } else {
            ...
            var infoWindowOptions = geoXML3.combineOptions(parserOptions.infoWindowOptions, {
                ...
                content: '<div id="iw-container">\
                                    <div class="iw-title" style="background-color: rgba(213,133,18,0.8) !important;">\
                                        ...
                                        <b style="float: left">' + fileToLayerDict[doc.url] + '</b>\
                                    </div>\
                                    <div style="max-height:200px;overflow-x: hidden;margin-bottom:3px; margin-left:10px;">\
                                    <div class="QuickInfoDesc" style="overflow:hidden;">'+
                                     (placemark.description != "" ? "<h3>" + placemark.name + "</h3></br>" + placemark.description : (placemark.vars.val.description != "" ? placemark.name + "</br>" + placemark.vars.val.description : "<div>" + placemark.name + "</div>")) +
                                        '</div>\
                                        </div>\
                                        </div>',
    ...
    var createPolyline = function (placemark, doc) {
        ...
        if (!parserOptions.suppressInfoWindows) {
            ...
            google.maps.event.addListenerOnce(p.infoWindow, 'domready', function () {
...
function setCustomInfoWindow() {

    var quickInfoDiv = $(".QuickInfoDesc");
    var indexChild = quickInfoDiv.children().length;

    if (quickInfoDiv.children()[indexChild - 1].scrollWidth > quickInfoDiv.children()[indexChild - 1].offsetWidth) {
        $($(".QuickInfoDesc").parent()).css("overflow-x", "scroll");
        $($(".QuickInfoDesc")).css({ "max-width": "none" });
        $($(".QuickInfoDesc")).css({ "width": 2 * quickInfoDiv.children()[indexChild - 1].scrollWidth - quickInfoDiv.children()[indexChild - 1].offsetWidth + "px" });

    } else {
        $(".QuickInfoDesc").css({ "max-width": "327px !important" });
        $($(".QuickInfoDesc").parent()[0]).css({ "overflow-x": "hidden !important" });
