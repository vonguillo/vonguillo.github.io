...
GooglePlaceMarkers.prototype = {
    ...
    searchFeature: function (searchBnd, searchStr, searchObj) {
        ...
        service.nearbySearch(requestObj, searchObj.callback);
    },
    ...
    callback: function (results, status) {
        ...
        if (status == google.maps.places.PlacesServiceStatus.OK) {
            delayPlaces = 50;
            console.log(results);
            callerObject.searchPlaces(results);


        } else if (status == google.maps.places.PlacesServiceStatus.OVER_QUERY_LIMIT) {
            delayPlaces = delayPlaces + 2000;
            setTimeout(callerObject.searchFeature(searchBnds, searchString, callerObject), delayPlaces);

        } else if (status == google.maps.places.PlacesServiceStatus.ZERO_RESULTS) {

            $("#layerspinner").css('visibility', 'hidden');
            var obj = { color: redColor + " !important;", html: "No " + $('label[for=' + callerObject.layerName + "label" + ']').html() + " data in the viewport area", visibility: "hidden" };
            rsgislayer.loadDisclaimer(obj);

        } else {

            $("#layerspinner").css('visibility', 'hidden');
            alert("CALLBACK in <searchFeature>: " + status);

            var obj = { color: redColor + " !important;", html: $('label[for=' + callerObject.layerName + "label" + ']').html() + " with status "+status, visibility: "hidden" };
            rsgislayer.loadDisclaimer(obj);

            //alert("CALLBACK in <searchFeature>" + callerObject.layerName + ": " + status);
        }
  
    },

    searchPlaces: function (results) {

        for (var i = 0; i < results.length; i++) {
            console.log(results[i]);
            if (results[i].types[0] == searchString) {
                callerObject.placesArray.push(results[i].place_id);
            } else if (callerObject.layerName == 'ambulance' && results[i].types[0] == 'health') {
                callerObject.placesArray.push(results[i].place_id);
            } else {
                //do nothing.
            }
        ...
        if (callerObject.placesArray.length != 0) {

            console.log("DELAY in Callback (before getDetail):" + delay);

            setTimeout(callerObject.getMarkerDetail(callerObject.placesArray.pop(), callerObject),0);
        }
        else {
            $("#layerspinner").css('visibility', 'hidden');
            var obj = { color: redColor + " !important;", html: "No " + $('label[for=' + callerObject.layerName + "label" + ']').html() + " data in the viewport area", visibility: "hidden" };
    ...
    getMarkerDetail: function (placeId, objLayer) {
        // https://maps.googleapis.com/maps/api/place/details/json?placeid=ChIJ24MzG_GwEmsRd2VLWl01368&key=AIzaSyDfh81aWxeHSIKDmMOQJYCvhhznAfFG1Eo
        ...
        var service = new google.maps.places.PlacesService(map);
        //console.log("Delay at getMarkerDetail: " + delay + " [ms]");
        service.getDetails(request, function (place, status) {
            ...
            if (status == google.maps.places.PlacesServiceStatus.OK) {
                ...
                var marker = new google.maps.Marker({
                    ...
                    icon: objLayer.getIconImage(objLayer.layerName)
                });

                var labels =['Name','Address','Phone','International Phone','More Info.']
                var fields = [place.name, place.formatted_address, place.formatted_phone_number, place.international_phone_number,place.website];
                //console.log(fields);
                var html = callerObject.getHTMLlabels(callerObject.layerName,fields,labels);
                                
                //******************** INFO WINDOW (to be activated) ***************
                google.maps.event.addListener(marker, 'click', function () {
                    ...
                    callerObject.popInfoWindow(html,place.geometry.location);

                });

                callerObject.geomArray.push(marker);

                if (callerObject.placesArray.length != 0) {

                    setTimeout(callerObject.getMarkerDetail(callerObject.placesArray.pop(), callerObject), delay);
                    //console.log("OK");
                    delay = delay/2;
                    ...
                    $("#layerspinner").css('visibility', 'hidden');
                    callerObject.setVisLayerOn(callerObject);
                ...
                delay = delay + 10000;
                console.log("OVERQUERY LIMIT - Delay: " + delay + "[ms]");
                setTimeout(callerObject.getMarkerDetail(placeId, callerObject),delay);

            } else {
                $("#layerspinner").css('visibility', 'hidden');
                alert("CALLBACK in <getMarkerDetail> " + callerObject.layerName + " : " + status);
                //console.log("CALLBACK in <getMarkerDetail> " + callerObject.layerName + " : " + status);
            }
        });
    },
    ...
    setVisLayerOff: function (self) {
        ...
        for (var i = 0; i < self.geomArray.length; i++) {
            ...
            //self.geomArray.pop().setMap(null);
        }
        return true;
    },

    setVisLayerOn: function (object) {
        //alert(9);
        //alert("setVisLayerON");
        //console.log(object.geomArray);
        for (var i = 0; i < object.geomArray.length; i++) {
            //console.log(self.geomArray[i]);
            object.geomArray[i].setMap(object.mapId);
        }
        //return true;
    },

    getIconImage: function (layername) {
        return {
            url: this.iconPath,
    ...
    getGeomArray: function () {

        return this.geomArray;
    },

    setMarkerTest: function (layerObj) {

        // alert("[function from Object] - setMarkerTest: " + layerObj.layerName);

        if (layerObj.layerName === "police") {

            var markerTest = new google.maps.Marker({
                position: { lat: 23.011373, lng: -102.013828 },
                icon: layerObj.getIconImage(layerObj.layerName),
                title: 'POLICE'
            });
        } else if ((layerObj.layerName === "hospital")) {
            var markerTest = new google.maps.Marker({
                position: { lat: 23.011373, lng: -101.853828 },
                icon: layerObj.getIconImage(layerObj.layerName),
                title: 'HOSPITAL'
            });
        } else {
            var markerTest = new google.maps.Marker({
                position: { lat: 23.011373, lng: -102.703828 },
                icon: layerObj.getIconImage(layerObj.layerName),
                title: 'OTHER'
            });
        ...
        layerObj.geomArray.push(markerTest);
        return true;
    },

    popInfoWindow: function (html, latLng) {
    ...
    //     getHTMLlabels(layername, fields,labels);
    getHTMLlabels: function (layername, fields, labels) {

        var iwTitle = $('label[for=' + layername + "label" + ']').html();
        if (mobileDevice) $("#feature-title").html(iwTitle);
        ...
        for (var i = 0; i < labels.length; i++) {
            ...
            if (labels[i] == "More Info.") {

                if (fields[i] != null) {

                    htmlString += '<a href = "' + fields[i] + '" target="_blank">' + labels[i] + '</a>';
                }

                
            }

            else if (labels[i] == 'Phone' || labels[i] == 'International Phone') {

                if (fields[i] != null) {

                    htmlString += "<b>" + labels[i] + ": </b>" + fields[i] + "</br>";
                }
...
function getPointLocation(mapCenter) {
    ...
    var point = turf.point([mapCenter.lng(), mapCenter.lat()]);

    insideFlag = turf.inside(point, usMultipolygon);
    if (insideFlag == true) { return 'US'; }
    else { return 'Other'; }


}

function getObjectByLayer(layername) {
    //console.log('-----------------------\n' + layername);
    for (var i = 0; inputParameters.length; i++) {
        if (inputParameters[i].layerName == layername) {
            ...
            return window.inputParameters[i];
        }
    }
}
